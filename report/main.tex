...
\newcommand{\TB}[1]{\textcolor{blue}{\mathbf{#1}}}
...
Le code, les configurations et les scripts d'évaluation sont disponibles sur GitHub.\footnote{\url{https://github.com/Dolgalad/HiParis_B2Ideas_challenge}. Le README contient des analyses complémentaires, des figures additionnelles et les instructions de reproduction. Expériences réalisées sur GPU NVIDIA A40. ChatGPT a été utilisé comme assistant pour le code, les figures et la rédaction, avec vérification manuelle des expériences, métriques et choix méthodologiques.} 
...
& Epoch & Micro-F1 $\uparrow$ & Macro-F1 $\uparrow$ & mAP $\uparrow$ \\
\midrule
ResNet-50       & $30$ & $0.4602$ & $0.3129$ & $0.3065$ & $49$ & $0.4774$ & $0.3440$ & $0.3466$ \\
CLIP ViT-B/32   & $23$ & $\LB{0.5632}$ & $\LB{0.4130}$ & $\LB{0.4203}$ & $45$ & $\TB{0.5720}$ & $\TB{0.4138}$ & $\TB{0.4385}$ \\
...
Les résultats de la Table~\ref{tab:stage-results} montrent que CLIP obtient les meilleures performances, avec un micro-F1 de $0.5720$, un macro-F1 de $0.4138$ et une mAP de $0.4385$ après fine-tuning.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth,height=0.2\textheight]{figures/test_predictions_grid.png}
...
La Figure~\ref{fig:per_genre_f1} détaille les performances par genre : CLIP domine sur la plupart des classes, en particulier les genres visuellement marqués comme \textit{animation}, \textit{family} ou \textit{horror}, tandis que les scores restent faibles pour les classes rares ou ambiguës comme \textit{documentary}, \textit{music} et \textit{tv movie}.

\begin{figure}[h]
    \centering
    \includegraphics[height=0.18\textheight,width=\linewidth]{figures/per_genre_f1.png}
    \vspace{-0.15em}
    \caption{F1-score par genre pour les trois modèles évalués sur le jeu de test.}
    \label{fig:per_genre_f1}
\end{figure}
